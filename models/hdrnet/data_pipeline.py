...
#       raise ValueError("Invalid data path.")
    ...
    input_files = [os.path.join(dirname, 'image', f) for f in flist]
    output_files = [os.path.join(dirname, 'label', f) for f in flist]
#     heatmap_files = [os.path.join(dirname, 'face_rect/heatmap', f) for f in flist]
#     rect_file = [os.path.join(dirname,'rect',f[:-3]+'xml') for f in flist]
...
#     target_width = []
#     for i in rect_file:
#         rect = xml.dom.minidom.parse(i)
#         rect = rect.documentElement
#         xmin = rect.getElementsByTagName('xmin')
#         xmax = rect.getElementsByTagName('xmax')
#         ymin = rect.getElementsByTagName('ymin')
#         ymax = rect.getElementsByTagName('ymax')
...
#         offset_height.append(ymin)
#         offset_width.append(xmin)
#         target_height.append(ymax-ymin)
#         target_width.append(xmax-xmin)
...
#     heatmap_file = tf.io.read_file(heatmap_queue)
...
#     if os.path.splitext(heatmap_files[0])[-1] == '.jpg':
#       im_heatmap = tf.io.decode_jpeg(heatmap_file, channels=1)
#     else:
#       im_heatmap = tf.io.decode_png(heatmap_file, dtype=output_dtype, channels=1)
...
#       im_heatmap = tf.compat.v1.to_float(im_heatmap)/output_wl
    inout = tf.concat([im_input,im_output], 2)
#     if self.random_crop:
#       inout = tf.image.crop_to_bounding_box(inout, ori_h_queue, ori_w_queue, tar_h_queue, tar_w_queue)
