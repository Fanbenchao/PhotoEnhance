...
REGISTER_OP("BilateralSlice")
  ...
  .SetShapeFn([](::tensorflow::shape_inference::InferenceContext* c) {
      ::tensorflow::shape_inference::ShapeHandle input_tensor;
      ::tensorflow::shape_inference::ShapeHandle guide_tensor;
      ::tensorflow::shape_inference::ShapeHandle output;
      TF_RETURN_IF_ERROR(c->Subshape(c->input(0), -1, &input_tensor));
      TF_RETURN_IF_ERROR(c->Subshape(c->input(1), 0, &guide_tensor));
      TF_RETURN_IF_ERROR(c->Concatenate(guide_tensor, input_tensor, &output));
...
REGISTER_OP("BilateralSliceApply")
  ...
  .SetShapeFn([](::tensorflow::shape_inference::InferenceContext* c) {
      ::tensorflow::shape_inference::ShapeHandle grid_tensor;
      ::tensorflow::shape_inference::ShapeHandle guide_tensor;
      ::tensorflow::shape_inference::ShapeHandle input_tensor;
      ::tensorflow::shape_inference::DimensionHandle chan_grid;
      ::tensorflow::shape_inference::DimensionHandle chan_in;
      ::tensorflow::shape_inference::DimensionHandle chan_out;
      ::tensorflow::shape_inference::ShapeHandle output;
      bool *has_offset;
      TF_RETURN_IF_ERROR(c->GetAttr("has_offset", has_offset));
      TF_RETURN_IF_ERROR(c->Subshape(c->input(0), 0, &grid_tensor));
      TF_RETURN_IF_ERROR(c->Subshape(c->input(1), 0, &guide_tensor));
      TF_RETURN_IF_ERROR(c->Subshape(c->input(2), 0, &input_tensor));
      chan_grid = c->Dim(grid_tensor, -1);
      chan_in = c->Dim(input_tensor, -1);
      if(*has_offset)
        {
          const int64 dividend = c->Value(chan_grid);
          const int64 divisor = c->Value(chan_in);
          const int64 mod_ = dividend%(divisor+1);
          chan_out = c->MakeDim((dividend-mod_)/(divisor+1));
        }
        else
        {
          const int64 dividend = c->Value(chan_grid);
          const int64 divisor = c->Value(chan_in);
          const int64 mod_ = dividend%divisor;
          chan_out = c->MakeDim((dividend-mod_)/divisor);
        }
      TF_RETURN_IF_ERROR(c->Concatenate(guide_tensor, c->Vector(chan_out), &output));
      c->set_output(0, output);
      return Status::OK();
    });
