...
#       flist = [l.strip() for l in fid.readlines()]
    img_dir = os.listdir(self.path)
    if self.shuffle:
      random.shuffle(img_dir)

    input_files = [os.path.join(self.path, f, 'input.jpg') for f in img_dir]
    output_files = [os.path.join(self.path, f, 'expertC.jpg') for f in img_dir]
