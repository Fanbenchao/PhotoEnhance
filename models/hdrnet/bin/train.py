...
import data_pipeline_lupin as dp1
...
def main(args, model_params, data_params):
  ...
  # --- Train/Test datasets ---------------------------------------------------
  data_pipe1 = getattr(dp1, args.data_pipeline)
  with tf.compat.v1.variable_scope('train_data'):
    train_data_pipeline = data_pipe1(
        args.data_dir,
        shuffle=True,
  ...
  if args.eval_data_dir is not None:
    data_pipe2 = getattr(dp1, args.data_pipeline)
    with tf.compat.v1.variable_scope('eval_data'):
      eval_data_pipeline = data_pipe2(
          args.eval_data_dir,
          shuffle=False,
  ...
  with tf.name_scope('train'):
    with tf.compat.v1.variable_scope('inference'):
      prediction,bilateral_coeffs = mdl.inference(
  ...
  if args.eval_data_dir is not None:
    with tf.name_scope('eval'):
      with tf.compat.v1.variable_scope('inference', reuse=True):
        eval_prediction,_= mdl.inference(
    ...
    while True:
      ...
      try:
        ...
        if args.eval_data_dir is not None and since_eval > args.eval_interval:
          ...
          sv.summary_writer.add_summary(tf.compat.v1.Summary(value=[
            tf.compat.v1.Summary.Value(tag="psnr/eval", simple_value=p_)]), global_step=step)
          if p_ >= psnr_best:
            psnr_best = p_
            saver.save(sess,args.checkpoint_dir+'/best/psnr_best',global_step=step)
          log.info("  Evaluation PSNR = {:.1f} dB, Best PSNR = {:.1f} dB".format(p_,psnr_best))
...
if __name__ == '__main__':
  ...
  req_grp.add_argument('--checkpoint_dir', default= '/home/lupin/PhotoEnhance/hdrnet_lupin/hdrnet/checkpoint', help='directory to save checkpoints to.')
  req_grp.add_argument('--data_dir', default='/home/lupin/PhotoEnhance/dataset/SICE/Part1/train.txt', help='input directory containing the training .tfrecords or images.')
  req_grp.add_argument('--eval_data_dir', default='/home/lupin/PhotoEnhance/dataset/SICE/Part1/valid.txt', type=str, help='directory with the validation data.')
  ...
  model_grp.add_argument('--data_pipeline', default='ImageFilesDataPipeline', help='classname of the data pipeline to use.', choices=dp1.__all__)
