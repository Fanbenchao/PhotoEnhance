...
def get_input_list(path):
  ...
  if os.path.isdir(path):
    ...
    inputs = [os.path.join(dirname, 'image', im) for im in inputs]
...
#   with tf.compat.v1.Session(config=config) as sess:
  ...
  saver = tf.compat.v1.train.import_meta_graph(metapath)

#     model_params_new = utils.get_model_params(sess)
...
#   tf.compat.v1.reset_default_graph()
  ...
  t_fullres_input = tf.compat.v1.placeholder(tf.float32, (None, None, None, 3))
  t_lowres_input = tf.compat.v1.placeholder(tf.float32, (None, net_shape, net_shape, 3))
...
#   saver = tf.compat.v1.train.Saver()
  ...
  if args.debug:
    ...
    if len(guide) > 0:
      for i, g in enumerate(guide):
        maxi = tf.compat.v1.reduce_max(tf.abs(g))
  ...
  with tf.compat.v1.Session(config=config) as sess:
    ...
    for idx, input_path in enumerate(inputs):
      ...
      im_input = im_input.reshape(im_input.shape)
      lowres_input = lowres_input.reshape(lowres_input.shape)
      feed_dict = {
...
if __name__ == '__main__':
  ...
  req_grp.add_argument('--checkpoint_dir', default='/home/lupin/PhotoEnhance/hdrnet_lupin/hdrnet/checkpoint_no_batch_norm', help='path to the saved model variables')
  req_grp.add_argument('--input', default='/home/lupin/PhotoEnhance/hdrnet_lupin/dataset/test.txt', help='path to the validation data')
  req_grp.add_argument('--output', default='/home/lupin/PhotoEnhance/hdrnet_lupin/outputs', help='path to save the processed images')
  # Optional
  req_grp.add_argument('--lowres_input', default=None, help='path to the lowres, TF inputs')
  req_grp.add_argument('--hdrp', dest="hdrp", action="store_true", help='special flag for HDR+ to set proper range')
  req_grp.add_argument('--nohdrp', dest="hdrp", action="store_false")
  req_grp.add_argument('--debug', dest="debug", action="store_true", help='If true, dumps debug data on guide and coefficients.')
  req_grp.add_argument('--limit', type=int, help="limit the number of images processed.")
  ...
  # -----------------------------------------------------------------------------
  parser.set_defaults(hdrp=False, debug=False,batch_norm=False)
