...
from layers import (conv, fc, bilateral_slice_apply,bilateral_slice)
...
class HDRNetCurves(object):
  ...
  def inference(cls, lowres_input, fullres_input, params,
    ...
    return output,bilateral_coeffs
  ...
  def _guide(cls, input_tensor, params, is_training):
    ...
    with tf.name_scope('ccm'):
      ccm_bias = tf.compat.v1.get_variable('ccm_bias', shape=[nchans,], dtype=tf.float32, initializer=tf.constant_initializer(0.0))
    ...
    with tf.name_scope('curve'):
      ...
      guidemap = tf.reduce_sum(slopes*tf.nn.relu(guidemap-shifts), axis = 4)

    guidemap = slim.conv2d(
        inputs=guidemap,
  ...
  def _output(cls, im, guide, coeffs):
    with tf.device('/gpu:0'):
      # out = bilateral_slice(coeffs, guide, name='slice')
